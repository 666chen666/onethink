    ...
    <div class="container-fluid" >
        ...
        <?php if(is_array($notices)): $i = 0; $__LIST__ = $notices;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$notice): $mod = ($i % 2 );++$i;?><div class="row noticeList">
            <a href="<?php echo U('Home/Index/detail?id='.$notice['id']);?>">
                <div class="col-xs-2">
                    <img class="noticeImg" src="<?php echo ($notice["path"]); ?>" />
                </div>
                <div class="col-xs-10">
                    <p class="title"><?php echo ($notice["title"]); ?></p>
                    <p class="intro"><?php echo ($notice["description"]); ?></p>
                    <p class="info"><?php echo ($notice["view"]); ?> <span class="pull-right"><?php echo (date('Y-m-d h:i:s',$notice["create_time"])); ?></span> </p>
                </div>
            </a>
