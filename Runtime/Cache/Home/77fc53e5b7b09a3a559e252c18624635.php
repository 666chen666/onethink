...
<div class="main">
    ...
    <div class="container-fluid">
        <div id="box">
        <?php if(is_array($shops)): $i = 0; $__LIST__ = $shops;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$shop): $mod = ($i % 2 );++$i;?><div class="row noticeList">
            <a href="<?php echo U('Home/Index/detail?id='.$shop['id']);?>">
                <div class="col-xs-2">
                    <img class="noticeImg" src="<?php echo ($shop["path"]); ?>" />
                </div>
                <div class="col-xs-10">
                    <p class="title"><?php echo ($shop["title"]); ?></p>
                    <p class="intro"><?php echo ($shop["description"]); ?></p>
                    <p class="info"><?php echo ($shop["view"]); ?> <span class="pull-right"><?php echo (date('Y-m-d h:i:s',$shop["create_time"])); ?></span> </p>
                </div>
            </a>
...
<script>
    $(function(){
        ...
        $(".get_more").click(function(){
            $.post("<?php echo U('index/shop');?>",{p:p+1},function(data){
                if(data.status == 1){
                    p++;
                    var html = '';
